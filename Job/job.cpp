...
int Job::getNodesCount() const
{
    return nodesCount;
}

void Job::job_writeToFile(const char* inFile, const char* outFile)
{
    // Clear the container before use again
    nodesX = Eigen::VectorXf();
    nodesY = Eigen::VectorXf();
    loads = Eigen::VectorXf();
